...
function parsePost(source, filename) {
  ...
  if (!metadata.title || !metadata.date || !metadata.description) {
    throw new Error(`${filename} needs title, date, and description in its front matter.`);
  }
...
function postPage({ title, date, body }) {
...
<html lang="en">
  ...
  <body>
    ...
    <main id="main"><article class="post section-shell"><header class="post-header"><p class="eyebrow">Blog</p><h1>${safeTitle}</h1><p class="post-date">${escapeHtml(displayDate(date))}</p></header><div class="post-body">${renderMarkdown(body)}</div><a class="back-link" href="../blog.html">← All posts</a></article></main>
